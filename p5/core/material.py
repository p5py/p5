from . import p5, fill
from ..sketch.util import ensure_p3d, scale_colors
import numpy as np
...
def ambient(r, g, b):
    ...
    """
    ensure_p3d('ambient')
    p5.renderer.ambient = np.array(scale_colors(r, g, b), dtype=np.float32)
...
def emissive(r, g, b):
    ...
    """
    ensure_p3d('emissive')
    return diffuse(*scale_colors(r, g, b))
...
def diffuse(r, g, b):
    ...
    """
    ensure_p3d('diffuse')
    p5.renderer.diffuse = np.array(scale_colors(r, g, b), dtype=np.float32)
...
def specular(r, g, b):
    ...
    """
    ensure_p3d('specular')
    p5.renderer.specular = np.array(scale_colors(r, g, b), dtype=np.float32)
